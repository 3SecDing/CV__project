...
def conv_blocks(in_channels, out_channels, kernel_size, stride, padding, groups=1):
    blocks = []
    blocks.append(nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=padding, groups=groups))
    blocks.append(LReLU())
...
class NormalModel(nn.Module):
    def __init__(self, upsample_type='deconv'):
        ...
        for i in range(len(channels)):
            self.add_module(f'layer{i + 1}', conv_blocks(in_channels, channels[i], 3, 1, 1))
            in_channels = channels[i]

        for i in range(len(channels))[::-1]:
            if i == 0:
                continue
            self.add_module(f'up_layer{i}', conv_blocks(channels[i], channels[i - 1], 3, 1, 1))
        ...
        if upsample_type == 'deconv':
            ...
            for i in range(len(channels) - 1):
                self.add_module(f'up{i + 1}', nn.UpsamplingBilinear2d(scale_factor=2))
        else:
